...
describe('Test Calc', () => {
  ...
  it('Test inputOperand1 by keyboard', () => {
    const wrapper = mount(Calc)
    const checkboxKeyboard = wrapper.find('input[name=screenKeyboard]')
    checkboxKeyboard.setChecked()
    const radioOperand = wrapper.find('input[name=checkedOperand1]')
    radioOperand.setChecked()
    const buttonNumberPush = wrapper.find('button[name="1"]')
    buttonNumberPush.trigger('click')
    expect(wrapper.vm.operand1).toBe(1)
  })

  it('Test inputOperand2 by keyboard', () => {
    const wrapper = mount(Calc)
    const checkboxKeyboard = wrapper.find('input[name=screenKeyboard]')
    checkboxKeyboard.setChecked()
    const radioOperand = wrapper.find('input[name=checkedOperand2]')
    radioOperand.setChecked()
    const buttonNumberPush = wrapper.find('button[name="3"]')
    buttonNumberPush.trigger('click')
    expect(wrapper.vm.operand2).toBe(3)
  })

  it('Test delete Operand1 by keyboard', () => {
    const wrapper = mount(Calc)
    const checkboxKeyboard = wrapper.find('input[name=screenKeyboard]')
    checkboxKeyboard.setChecked()
    const radioOperand = wrapper.find('input[name=checkedOperand1]')
    ...
    const buttonNumberPush = wrapper.find('button[name="<-"]')
    buttonNumberPush.trigger('click')
    expect(wrapper.vm.operand1).toBe(12)
  })

  it('Test delete Operand2 by keyboard', () => {
    const wrapper = mount(Calc)
    const checkboxKeyboard = wrapper.find('input[name=screenKeyboard]')
    checkboxKeyboard.setChecked()
    const radioOperand = wrapper.find('input[name=checkedOperand2]')
    radioOperand.setChecked()
    const operand2 = wrapper.find('input[name=operand2]')
    operand2.setValue('789')
    const buttonNumberPush = wrapper.find('button[name="<-"]')
    buttonNumberPush.trigger('click')
    expect(wrapper.vm.operand2).toBe(78)
  })
})
